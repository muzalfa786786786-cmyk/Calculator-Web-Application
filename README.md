# Calculator-Web-Application
 I have successfully developed a fully functional Scientific Calculator Web Application using HTML, CSS, and JavaScript.
During this project, I worked on:
```
✅ Basic arithmetic operations (+, −, ×, ÷)
✅ Scientific functions (sin, cos, tan, log, √, π)
✅ JavaScript Math methods integration
✅ DOM Manipulation & Event Handling
✅ Responsive and modern UI design
```
This hands-on task has helped me improve my practical skills in JavaScript logic building, dynamic calculations, and interactive UI development.
Looking forward to exploring APIs and enhancing UI in the upcoming tasks!
---

# 🔗 Live Demo

🌐 [https://calculator-web-application-kfzerfrah.vercel.app ](https://calculator-web-application-kfzerfrah.vercel.app) 

---
#FrontendDevelopment 
#JavaScript 
#WebDevelopment 
#DOMManipulation
